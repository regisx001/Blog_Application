...
public abstract class UserMapperRef {
    ...
    public abstract UserDtoRef.Basic toBasicDto(User user);

    @Mapping(target = "avatar", source = "avatar", qualifiedByName = "avatarNameToFullUri")
    public abstract UserDtoRef.Profile toProfileDto(User user);

    @Mapping(target = "avatar", source = "avatar", qualifiedByName = "avatarNameToFullUri")
    @Mapping(target = "roles", source = "roles", qualifiedByName = "mapRolesToNames")
    public abstract UserDtoRef.Detailed toDetailedDto(User user);

    @Mapping(target = "avatar", source = "avatar", qualifiedByName = "avatarNameToFullUri")
    @Mapping(target = "articlesCount", expression = "java(calculateArticlesCount(user))")
    public abstract UserDtoRef.WithCount toWithCountDto(User user);

    @Mapping(target = "avatar", source = "avatar", qualifiedByName = "avatarNameToFullUri")
    @Mapping(target = "roles", source = "roles", qualifiedByName = "mapRolesToNames")
    @Mapping(target = "articlesCount", expression = "java(calculateArticlesCount(user))")
    public abstract UserDtoRef.Admin toAdminDto(User user);

    public abstract UserDtoRef.Option toOptionDto(User user);
    ...
    public abstract User toEntity(UserDtoRef.RegisterRequest request);
    ...
    protected String avatarNameToFullUri(String avatar) {
        if (avatar == null || avatar.isBlank()) {
            return baseUrl + "/uploads/avatars/default-avatar.png"; // default avatar
        }
